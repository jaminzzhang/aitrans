...
| 研发负责人 | [KNOWN] 待确认 |
| 测试负责人 | [KNOWN] 待确认 |
| 当前状态 | [KNOWN] `PARTIAL_VERIFICATION`；本地测试、静态分析、macOS Debug 构建与启动通过，移动端构建受本机工具链阻断 |
...
| [KNOWN] 凭证安全隔离 | [KNOWN] API Key 不进入普通 Hive，按 Provider ID 写入系统安全存储 | [KNOWN] Hive 中无 API Key；不同 Provider 读取不同凭证 |
...
| [KNOWN] Provider 凭证仓储 | [KNOWN] 使用系统安全存储，key 名只含稳定 Provider ID | [KNOWN] 用户已确认安全存储方案；项目规则禁止普通 Hive 保存凭证 |
...
| [KNOWN] 生物识别强制解锁 | [KNOWN] 会改变每次调用的交互与平台最低要求 | [KNOWN] 使用包默认非生物识别安全存储 |
...
|---|---|---|---|---|---|---|---|
| ROOT | 业务目标 | [KNOWN] 用户配置 AI Provider | [KNOWN] 非敏感设置与凭证分仓保存 | [KNOWN] 重启后恢复且不明文落盘凭证 | [KNOWN] 重建仓储与跨 Provider 测试 | P1 | [KNOWN] 已确认 |
...
| MAIN-4 | 保存 | [KNOWN] 点击保存且 Draft 合法 | [INFERRED] 写安全凭证与非敏感偏好；全部成功后更新全局状态并关闭 | [KNOWN] 新配置生效 | [KNOWN] 成功与任一步失败测试 | P1 | [KNOWN] 已确认 |
...
|---|---|---|---|---|---|---|
| SET-001 | Provider configuration | [KNOWN] 非敏感配置可持久化，API Key 不得写入普通 Hive | [KNOWN] Draft | [KNOWN] 分仓记录 | [KNOWN] 日志和错误不得包含凭证 | [KNOWN] 已确认 |
...
| SET-003 | Draft commit | [KNOWN] 编辑与测试不改变全局配置 | [KNOWN] 本地 Draft | [KNOWN] 保存成功后一次性更新 | [KNOWN] 跨存储无真正事务；失败时全局状态不变 | [KNOWN] 已确认 |
...
| 交易与数据一致性 | [KNOWN] 是 | [KNOWN] Hive 与系统凭证库是两个独立存储 | [KNOWN] 无跨存储事务 | P1 |
| 状态流转 | [KNOWN] 是 | [KNOWN] Draft、保存中、成功、失败、取消 | [KNOWN] 无 | P1 |
| 幂等与并发 | [KNOWN] 是 | [INFERRED] 重复保存应产生相同最终状态 | [KNOWN] 不支持多个设置窗口并发编辑 | P2 |
| 权限与审计 | [KNOWN] 是 | [KNOWN] 系统凭证库需要平台 entitlement/权限 | [KNOWN] 无 | P1 |
...
| 安全存储 | `lib/core/security/` | [INFERRED] 新增 Provider 凭证接口与插件适配器 | P1 |
...
| 平台配置 | macOS/iOS entitlements、Android min SDK | [KNOWN] 安全存储插件的平台要求 | P1 |
| 依赖 | `pubspec.yaml`, `pubspec.lock` | [KNOWN] 新增稳定版 `flutter_secure_storage` | P1 |
...
| [KNOWN] macOS 调试构建 | 平台验证 | P1 | [KNOWN] Keychain entitlement 必须真实生效 |
