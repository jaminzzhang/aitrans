...
| 检查项 | 状态 | 证据 |
|---|---|---|
| Service bundle 声明 | [KNOWN] PASS | [COMPUTED] 编译后 `Info.plist` 含菜单“使用 AITrans 翻译”、`translateSelection`、`NSStringPboardType` 与 `public.utf8-plain-text` send types 和 10 秒系统超时 |
...
| 稳定安装与注册 | [KNOWN] PASS | [COMPUTED] Debug bundle 安装到 `~/Applications/AITrans Debug.app` 后，执行 LaunchServices 强制注册和 `pbs -update`，live 数据库仍保留该 `NSBundlePath`；App 单实例启动并存活至少 2 秒 |
...
| TextEdit 公开测试文本 | [COMPUTED] PASS | [KNOWN] 未触发 | [KNOWN] 未验证 | [KNOWN] 不适用 | [KNOWN] 不适用 | [KNOWN] `PARTIAL_VERIFICATION`；有效选区菜单已出现 |
...
- [KNOWN] 历史验证中的最终签名 Debug App 曾在当前机器启动后无窗口，主线程采样持续停在 `_libsecinit_appsandbox` 等待 XPC；S6 稳定安装流程已恢复进程启动与存活，但尚未据此推断窗口和宿主菜单通过。
